...
def structure_transcript(raw: str, anthropic_key: str,
                         speaker: str = "", topic: str = "",
                         scripture: str = "", chapter: str = "",
                         verse_range: str = "",
                         custom_prompt: str = "") -> str:
    """Transform raw transcript into full structured document using Claude SDK."""
    import anthropic
    client = anthropic.Anthropic(api_key=anthropic_key)
    ...
    if custom_prompt and custom_prompt.strip():
        custom_note = (
            "\n\n==============================================================\n"
            "SPECIAL FOCUS INSTRUCTIONS — apply throughout the entire transcript:\n"
            "==============================================================\n"
            + custom_prompt.strip()
            + "\n\nHOW TO APPLY THESE INSTRUCTIONS:\n"
            "- Use these instructions to guide HOW you structure and present the content.\n"
            "- Do NOT insert any inline labels or annotations like [KEY EMPHASIS], "
            "[CROSS-REFERENCE], [ANALOGY], or [SANSKRIT TERM] into the text.\n"
            "- Instead, reflect the focus naturally in the structure:\n"
            "  * For stressed points: use bold text (**like this**) or place them "
            "prominently as section headings or opening sentences of paragraphs.\n"
            "  * For cross-references: weave them naturally into the text "
            "(e.g. 'As referenced in the Bhagavatam...' or cite inline).\n"
            "  * For analogies: present them clearly in their own paragraph with "
            "a natural lead-in sentence.\n"
            "  * For Sanskrit terms: include them naturally with meaning in parentheses.\n"
            "  * For quotable statements: present them as standalone paragraph or "
            "blockquote (> text) so they stand out naturally.\n"
            "The output must read as clean, beautiful, flowing prose — "
            "with the focus instructions shaping the content, not labelling it.\n"
        )

    prompt = (
        f"You are an expert Vedantic discourse transcriber.\n"
        f"Speaker: {speaker or 'Not specified'} | Topic: {topic or 'Not specified'} | "
        f"Scripture: {scripture or 'Not specified'} | "
        f"Chapter: {chapter or 'Not specified'} | "
        f"Verses: {verse_range or 'Full discourse'}\n\n"
        "Transform this raw transcript into a complete, beautifully structured document.\n"
        "RULES:\n"
        "1. Include EVERY sentence — do NOT skip or summarize anything.\n"
        "2. Fix grammar and remove filler words only. Preserve all content.\n"
        "3. Organize with ## section headings (Introduction, Main Teaching, "
        "Scriptural Explanation, Story/Analogy, Practical Application, etc.).\n"
        "4. Sanskrit verses MUST appear in Devanagari script only — never Roman.\n"
        "5. End with: ## Concise Summary | ## Key Takeaways | "
        "## Sanskrit Terms Glossary | ## Practical Reflection\n"
        "6. Do NOT use inline annotation labels — write clean flowing prose throughout.\n"
        + custom_note
        + "\nRaw transcript:\n\n"
    )
    ...
    while pos < len(raw):
        ...
        pos = end
        note = (
            f"\n[Part {chunk_num}/{total} - structure fully, no end sections yet]\n"
            if chunk_num < total else
        ...
        parts.append(("\n\n---\n\n" + msg.content[0].text) if chunk_num > 1 else msg.content[0].text)
    return "\n".join(parts)
