...
if om_path.exists():
    try:
        om_src = _img_src(str(om_path))
        st.markdown(
            f"<div style='text-align:center;width:100%;'>"
            f"<img src='{om_src}' style='width:80px;height:80px;"
            f"object-fit:contain;display:inline-block;margin:0 auto;'/></div>",
            unsafe_allow_html=True
        )
    except Exception:
...
if gurudev_path.exists():
    ...
    gurudev_sq.save(buf, format='JPEG', quality=90)
    import base64 as _b64g
    g_src = "data:image/jpeg;base64," + _b64g.b64encode(buf.getvalue()).decode()
    st.markdown(
        f"<div style='text-align:center;width:100%;'>"
        f"<img src='{g_src}' style='width:190px;height:190px;object-fit:cover;"
        f"border-radius:50%;border:3px solid #c9a96e;"
        f"box-shadow:0 0 24px rgba(201,169,110,0.35);display:inline-block;margin:0 auto;'/>"
        f"</div>",
        unsafe_allow_html=True
    )
swami_label(
...
<div style='display:flex;justify-content:center;align-items:flex-start;
            gap:60px;padding:0.5rem 0 1rem;flex-wrap:wrap;'>
  <div style='text-align:center;flex:0 0 auto;'>
    ...
    <div style='font-size:0.75rem;color:#c9a96e;letter-spacing:0.4px;'>Chinmaya Mission Chicago</div>
  </div>
  <div style='text-align:center;flex:0 0 auto;'>
