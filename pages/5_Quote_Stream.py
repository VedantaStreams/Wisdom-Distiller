...
st.set_page_config(
    page_title="Quote Stream · Wisdom Distiller",
    page_icon="🌊",
    layout="centered",
...
st.markdown("""
<div class="hero">
    <h1>🌊 Quote <span class="accent">Stream</span></h1>
    <p class="subtitle">Verbatim wisdom · YouTube titles · Reels · Hashtags</p>
</div>
    ...
    with dc2:
        try:
            ...
            st.download_button("📕 PDF", pdf,
                               file_name="quote_stream.pdf", mime="application/pdf")
        except Exception as e:
            st.caption(f"PDF: {e}")
    with dc3:
        try:
            docx = make_docx("Quote Stream", export_text,
                             speaker=result.get("speaker",""),
                             topic=result.get("topic",""),
                             scripture=result.get("scripture",""))
            st.download_button("📘 DOCX", docx,
                               file_name="quote_stream.docx",
...
# ── Step 3 — Extract ──────────────────────────────────────────────────────────
st.markdown("<br/>", unsafe_allow_html=True)
if st.button("🌊 Extract Quotes", key="qs_process", use_container_width=True):
