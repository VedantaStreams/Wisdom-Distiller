...
if st.button("📜 Transcribe & Structure", key="dt_process", use_container_width=True):
    if not openai_key:
        ...
        try:
            ...
            # Prepend metadata so Claude knows context
            meta_header = (
                f"[Speaker: {speaker} | Topic: {topic} | "
                f"Scripture: {scripture} | Chapter: {chapter} | Verses: {verse_range}]\n\n"
            )
            annotated = meta_header + raw
            if mode == "📋 Summary only":
                style = "Detailed paragraphs"
            else:
                style = "Academic digest"
            structured = summarize_text(annotated, style, [], anthropic_key)
