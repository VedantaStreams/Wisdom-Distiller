...
def _get_visit_count():
    """Read current visit count from Google Sheet."""
    try:
        import gspread
        ...
        try:
            ...
            ws = sheet.add_worksheet("Visitors", rows=10, cols=3)
            ws.append_row(["Total Visits", "Unique Sessions"])
            ws.append_row([0, 0])
        row = ws.row_values(2)
        visits   = int(row[0]) if row and row[0] else 0
        sessions = int(row[1]) if len(row) > 1 and row[1] else 0
        return visits, sessions, ws
    except Exception:
        return None, None, None

def _increment_visit(ws, visits, sessions):
    """Increment visit count in Google Sheet."""
    try:
        ws.update("A2", [[visits + 1, sessions + 1]])
    except Exception:
        pass
...
try:
    if "page_visit_counted" not in st.session_state:
        ...
        visits, sessions, ws = _get_visit_count()
        if ws is not None:
            _increment_visit(ws, visits, sessions)
        ...
        st.session_state["_total_sessions"] = total_sessions
    else:
        total_visits   = st.session_state.get("_total_visits", "—")
        total_sessions = st.session_state.get("_total_sessions", "—")
